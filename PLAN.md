...
| Устройства | **Каждое устройство — своя ссылка (свой слот), не больше 1 одновременной сессии на ссылку** (`limits.max_sessions = 1` в профиле). У человека может быть несколько устройств, лимит устройств задаётся на человека (по умолчанию 1, настройка + поле у пользователя). Срок и лимит трафика — общие на человека. Устройства добавляют бот (сам пользователь, в пределах лимита) и панель (админ). |
...
**Слот** — это место для одного **устройства** пользователя: свой секрет, свой loopback-порт, свой профиль в relay с `limits.max_sessions = 1`.
...
| Выдать доступ / добавить устройство | взять **чистый** свободный слот, удалить порт из `@blocked` | нет |
| Выключить (пользователя) | порты всех его устройств в `@blocked` + `ss -K` на эти порты (обрыв текущих потоков) | нет |
| Включить | удалить порты его устройств из `@blocked` | нет |
| Перевыпустить ссылку (устройства) | устройство переезжает на новый чистый слот, старый: выключить → `dirty` | нет |
| Удалить устройство / пользователя | выключить, слот(ы) → `dirty` | нет |
...
slots(id INTEGER PK, shard_id, idx, port UNIQUE, secret, status CHECK(status IN('free','assigned','dirty')), device_id NULL)
users(
  ...
  max_devices INTEGER NULL,                           -- NULL = значение из настроек (по умолчанию 1)
  enabled, disabled_reason NULL,                      -- manual|expired|traffic_limit
...
devices(id INTEGER PK, user_id, name, slot_id UNIQUE, created_at, created_by,   -- panel|bot
        total_up, total_down)                                                 -- трафик устройства за всё время
...
traffic_hourly(user_id, device_id, hour, up, down, PRIMARY KEY(device_id,hour))   -- хранить 90 дней; по человеку — сумма
...
```
Индексы: `users(enabled)`, `users(expires_at)`, `users(name)`, `devices(user_id)`, `traffic_hourly(user_id, hour)`, `requests(status)`, `slots(status)`.
...
5. При одобрении создаётся пользователь (`name` = имя из Telegram, можно поправить в панели) и его первое устройство (выдаётся слот), бот отправляет пользователю ссылку, QR (PNG) и короткую инструкцию: «одна ссылка — одно устройство; для другого устройства нажмите „Добавить устройство“».
...
- `tg_id` уникален: один Telegram-аккаунт — один пользователь. **Одна ссылка = одно устройство** (в каждый момент работает только одна сессия по ссылке). Форма создаёт первое устройство; лимит устройств — поле «Макс. устройств» (пусто = из настроек).
...
`/start`, `/id`, `/link` (прислать ссылки и QR всех своих устройств снова, если доступ активен), `/devices` (список устройств; кнопки «➕ Добавить устройство» — сразу выдаёт новую ссылку, если не превышен лимит устройств и доступ активен, иначе вежливый отказ; «🗑 Удалить» у каждого устройства — слот → `dirty`, освобождает место под другое устройство), `/status` (срок, трафик/лимит, состояние, устройства), `/help`. Главное меню — reply-клавиатура с этими пунктами.
...
2. `delta = cur - last` (если `cur < last`, значит таблица пересоздавалась → `delta = cur`). Дельта начисляется устройству, которое **сейчас** владеет слотом, и его пользователю (срок и лимит трафика — на пользователя, по сумме устройств). При назначении слота `counters_last` для порта сбрасывается на текущее значение.
3. Одна транзакция: обновить `users.period_*`, `users.total_*`, `devices.total_*`, `traffic_hourly`.
4. Проверки:
...
3. **Пользователь:** параметры (в т. ч. «Макс. устройств»), список устройств (у каждого: имя, ссылка + QR, трафик, «перевыпустить ссылку», «отправить в Telegram», «удалить»; кнопка «Добавить устройство»), график трафика по часам/дням за 30 дней (сумма и по устройствам), действия (вкл/выкл, продлить, лимит, удалить), журнал событий пользователя.
...
7. **Настройки:**
   ...
   - Пул: размер (добавить шарды), время окна обслуживания. `max_sessions` профиля всегда 1 (одна ссылка — одно устройство), не настраивается.
   - Устройства: лимит устройств на человека по умолчанию (1).
...
- `devices_per_user` ≈ 1,2 (по умолчанию лимит 1, часть пользователей получит больше); одна сессия на устройство;
...
pool_slots        = ceil(users_target * devices_per_user * 1.1 / 16) * 16
max_profiles      = pool_slots + 16 (запас)
max_sessions_global = pool_slots   (у каждого слота не больше 1 сессии; проверить, что проходит -check по резерву)
...
**M1 — ядро.** `db`, `links`, `pool`, `system`, `ctl`, `users` (с устройствами), шаблон шарда, nft-таблица. Тесты. **Критерий:** на тестовом VPS 20 шардов / 320 слотов; выдача, выключение и включение не рвут соединения других; выключенный пользователь отваливается за секунды; ссылки с base_path и без работают в Telegram с поддержкой WEB proxy; вторая одновременная сессия по одной ссылке отклоняется.
